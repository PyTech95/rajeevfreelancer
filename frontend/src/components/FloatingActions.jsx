...
function Modal({ title, subtitle, onClose, children, testid }) {
  return (
    <motion.div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-4"
      ...
      <motion.div className="absolute inset-0 bg-ink/85 backdrop-blur-sm" onClick={onClose} />
...
function CallScheduler({ prefillService, location, onClose }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setDone(true);
      navigate("/thank-you", { state: { name: f.name, service: "Call booking" } });
    } catch (err) {
    ...
    <>
      <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
        <motion.button type="button" onClick={() => setModal("inquiry")} data-testid="floating-inquiry-btn"
          ...
          className="flex items-center gap-2.5 rounded-full bg-brand text-white p-3 sm:pl-4 sm:pr-5 sm:py-3 shadow-lg shadow-brand/30" aria-label="Send an inquiry">
          <MessageSquarePlus className="h-5 w-5" /><span className="hidden sm:inline text-sm font-semibold">Inquiry</span>
        </motion.button>
