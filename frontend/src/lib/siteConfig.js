...
export const SITE_DEFAULTS = {
  seo: {
    ...
    default_description:
      "Hire Rajeev — app developer, website development & SEO marketing consultant in Delhi NCR. 12+ years turning apps, SEO & AI automation into revenue. Free quote.",
...
// Injects arbitrary HTML (incl. <script> tags, which need re-creation to execute).
function injectHtml(target, html, markerId) {
...
// Inject all configured tracking once: gtag (GA4/Ads), GTM, Meta Pixel + admin custom code.
export function initTracking() {
  ...
  if (t.meta_pixel_id) {
    /* eslint-disable */
    !function(f,b,e,v,n,ts,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version="2.0";n.queue=[];ts=b.createElement(e);ts.async=!0;ts.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(ts,s)}(window,document,"script","https://connect.facebook.net/en_US/fbevents.js");
    /* eslint-enable */
    window.fbq("init", t.meta_pixel_id);
    window.fbq("track", "PageView");
  }

  injectHtml(document.head, t.head_code, "rf-custom-head-code");
  injectHtml(document.body, t.body_code, "rf-custom-body-code");
}

// SPA page-view: keeps GA4/Ads/Pixel aware of route changes (needed for URL-based
// conversion rules like "/thank-you" page views).
export function firePageView(path) {
  const t = getSiteConfig().tracking || {};
  if (typeof window === "undefined") return;
  if (window.gtag) {
    const meta = { page_path: path, page_location: window.location.href, page_title: document.title };
    window.gtag("event", "page_view", meta);
    if (t.ads_id) window.gtag("config", t.ads_id, { page_path: path });
  }
  if (window.fbq) window.fbq("track", "PageView");
  if (window.dataLayer && t.gtm_id) window.dataLayer.push({ event: "spa_page_view", page_path: path });
}

// Fire a lead conversion (GA4 generate_lead + Google Ads conversion + Meta Lead) on form success.
export function trackConversion(meta = {}) {
  ...
  if (window.dataLayer) window.dataLayer.push({ event: "generate_lead", ...meta });
}

// Runs the admin-configured thank-you page snippet (conversion pixels etc.).
export function fireThankyouCode() {
  const t = getSiteConfig().tracking || {};
  injectHtml(document.body, t.thankyou_code, "rf-thankyou-code");
}
...
export function personSchema() {
  ...
  return {
    ...
    jobTitle: "App Developer, Website Development & SEO / Digital Marketing Consultant",
    description:
